...
#SOLVED: /foo/bar/foo or /foo/foo/foo
#this are all different foo's
fileSystem = 70000000
spaceRequired = 30000000
usedSpace = sizes["/"]
freeSpace = fileSystem - usedSpace
spaceToFree = spaceRequired - freeSpace

minimum = 30000000
for i in sizes:
    if min(minimum, sizes[i]) > spaceToFree:
        minimum = min(minimum, sizes[i])
print(minimum)
